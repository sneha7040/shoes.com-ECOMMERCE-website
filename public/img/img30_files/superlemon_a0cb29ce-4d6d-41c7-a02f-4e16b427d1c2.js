...
function initCss(done) {
  ...
  link.setAttribute(
    "href",
    "https://cdn.shopify.com/s/files/1/0070/3666/5911/files/superlemon_ef8af1ae-2a3f-44b4-b938-6b43a6f9eab8.css?v=1639566914"
  );
...
function btnLoad() {
  ...
  const SERVER_BASE_URL_MESSAGING = "http://optin-api.superlemon.xyz";
  const SERVER_BASE_URL_ANALYTICS = "http://analytics-api.superlemon.xyz";
  ...
  function getDeviceTypeFromSettings(settings, type = "chat") {
    ...
    const positionType = type === "chat" ? "chatPos" : "sharePos";
  ...
  function isShareBtnVisible(settings) {
    ...
    if (window.location.pathname === "/" && !showOnPages?.includes("HOME")) {
      ...
      (!showOnPages?.includes("CART_DESKTOP") ||
        !showOnPages?.includes("CART_MOBILE"))
    ) {
      return false;
    } else if (
      (window.location.pathname.match("(.*)/orders/(.*)") ||
